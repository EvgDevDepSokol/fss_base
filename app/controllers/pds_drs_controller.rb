...
class PdsDrsController < BaseController
  ...
  def pds_drs
    ...
    @eng_sys_list = {}
    @time_period = {}
    @time_period[:date_min] = PdsDrComment.where(project: project.ProjectID).minimum(:comment_date)
    @time_period[:date_max] = PdsDrComment.where(project: project.ProjectID).maximum(:comment_date)
