class DifferentStuffController < ApplicationController
  ...
  def compare_kursks
    hash = params[:data]
    ref1 = HwIc.where(Project: 80000001).pluck(:ref) #old
    ref3 = HwIc.where(Project: 80000003).pluck(:ref) #2014
    pag1 = ref1 - ref3
    pag2 = ref3 - ref1
    pag3 = []
    list3 = ref1 - (ref1 - ref3)
    hw_ic1_arr=hw_ic_by_ref_arr(80000001, list3)
    hw_ic3_arr=hw_ic_by_ref_arr(80000003, list3)
  ...
  def hw_ic_by_ref(proj, ref)
    sql = "SELECT  pds_syslist.System, `hw_ic`.`Description`, hw_peds.ped,hw_devtype.RuName, `hw_ic`.`scaleMin`, `hw_ic`.`scaleMax`, pds_unit.Unit_RU, `hw_ic`.`tag_no` FROM `hw_ic` LEFT OUTER JOIN `pds_project_unit` ON `pds_project_unit`.`ProjUnitID` = `hw_ic`.`Unit` LEFT OUTER JOIN `pds_unit` ON `pds_unit`.`UnitID` = `pds_project_unit`.`Unit` LEFT OUTER JOIN `hw_peds` ON `hw_peds`.`ped_N` = `hw_ic`.`ped` LEFT OUTER JOIN `hw_devtype` ON `hw_devtype`.`typeID` = `hw_peds`.`type` LEFT OUTER JOIN `pds_syslist` ON `pds_syslist`.`SystemID` = `hw_ic`.`sys` WHERE `hw_ic`.`Project` = #{proj} AND `hw_ic`.`ref` = '#{ref}'"
    e = ActiveRecord::Base.connection.execute(sql).first
    e1 = {}
    e1['sys']              = e[0]
    e1['desc']             = e[1]
    e1['ped']              = e[2]
