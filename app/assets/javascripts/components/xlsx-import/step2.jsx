    ...
    if (options.length <= 0) {
      ...
      for (var i = 0; i < options.length; i++) {
        if (!n[options[i].label]) {
          ...
          n[options[i].label].col =
            n[options[i].label].col + ', \'' + options[i].key + '\'';
        }
      }
      Object.keys(n).forEach(function(key) {
        if (n[key].cnt > 1) {
          message.push(
            'Поле импорта \'' +
              key +
              '\' выбрано несколько раз. Проверьте столбцы: ' +
              n[key].col +
              '.'
          );
        }
  ...
  afterOpenModal = () => {
    ...
    Object.keys(importHeaders).forEach(function(key) {
      ...
      options.forEach(function(opt) {
        if (opt['label'] == key) selectVal = opt['value'];
      });
      ...
      if (importHeaders[key]['to']) {
        options3.push({
          value: importHeaders[key]['toColumn']['attribute']
            ? importHeaders[key]['toColumn']['attribute']
            : importHeaders[key]['to'],
          label: importHeaders[key]['toColumn']['label']
            ? importHeaders[key]['toColumn']['label']
            : importHeaders[key]['to']
        });
      }
    });
  ...
  checkKeyColumn = () => {
    ...
    if (keyColumn) {
      Object.keys(importHeaders).forEach(function(key) {
        if (importHeaders[key]['to']) {
          if (
            importHeaders[key]['to'] == keyColumn ||
            importHeaders[key]['toColumn']['attribute'] == keyColumn
          ) {
            keyImport = key;
          }
        }
      ...
      if (n['undefined']) {
        message.push(
          'В некоторых записях файла ключевое поле является пустым.'
        );
      ...
      Object.keys(n).forEach(function(key) {
        if (n[key].cnt > 1 && key !== 'undefined') {
          message.push(
            'В файле встречаются записи с одинаковым ключом: \'' + key + '\'.'
          );
          ...
          >
            ...
            <div className="modal-table-container" key={'modal-table'}>
              ...
              >
                <thead>
                  <tr>{headersFrom}</tr>
                  <tr className="selector-header">{headersTo}</tr>
                </thead>
                <tbody>{rows}</tbody>
              </table>{' '}
            </div>{' '}
            ...
            <div>
              {' '}
              Выберите ключевое поле, по которому запись будет искаться в базе.{' '}
            </div>
            <div>
              {' '}
              Значение поля для каждой записи из импортируемого файла должно
              быть уникальным и не должно быть пустым.{' '}
            </div>
            <div>
              {' '}
              При возникновении сообщения об ошибке, внесите исправления в
