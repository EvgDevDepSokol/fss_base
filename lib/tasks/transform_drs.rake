namespace :transform_drs do
  task open_dr: [:environment] do
    ...
    pds_drs.each do |pds_dr|
      ...
      pds_dr_comment.comment_date = pds_dr.createDate - 1.year
  ...
  task approve_dr: [:environment] do
    pds_drs = PdsDr.all
    pds_drs.each do |pds_dr|
      pds_dr_comment = PdsDrComment.new
      pds_dr_comment.pds_dr_id = pds_dr.id
      pds_dr_comment.Project = pds_dr.Project
      pds_dr_comment.comment_date = pds_dr.createDate - 1.year + 1.day
      pds_dr_comment.comment_text = 'Принят в работу'
